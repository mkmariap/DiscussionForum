...
public class UserActivity {
    ...
    public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
        ...
        else if (user.getUserRole() == UserRole.MODERATOR) { // the user is moderator
            if (question.getUser().getUserRole() == UserRole.USER || question.getUser() == user) { // if the user who wrote the question is the current user (i.e., Moderator) or regular user
                questionService.deleteQuestion(question); // deleting a question
            }
             else {
    ...
    // Write else if condition to check if the user is a regular user
        ...
        else if (user.getUserRole() == UserRole.USER) { // the user is regular user
            if (question.getUser() == user) { // the current user is the user who wrote the question
                questionService.deleteQuestion(question); // deleting a question
            } else {
                System.out.println("You are not authorised to delete this question");
            }
